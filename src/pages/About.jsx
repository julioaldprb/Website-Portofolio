    ...
    >
        ...
        <div className="border-l-2 border-blue-200 ml-4">
          {timeline.map((item, idx) => (
            ...
            >
              ...
              <p className="text-gray-500 italic mb-0.5">{item.subtitle}</p>
      ...
      <motion.section variants={sectionVariant} transition={{ delay: 0.55 }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4">
          {[
            'Python','Machine Learning','NLP','React','Docker','Streamlit','Data Analysis','MLOps',
            'UI/UX','Video Editing','Web Development','Git','Canva','Figma','Next.js','HTML/CSS',
          ].map((skill, i) => (
            <motion.span
              key={skill}
              variants={itemVariant}
      ...
      <motion.section variants={sectionVariant} transition={{ delay: 0.75 }}>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {certificates.map((cert, idx) => (
            <motion.div
              key={idx}
              className="w-56 bg-white rounded-2xl shadow-md overflow-hidden cursor-pointer"
      ...
      {selectedCert && (
        ...
        >
          ...
          >
            <X
              className="absolute top-2 right-2 text-white w-6 h-6 cursor-pointer"
              onClick={() => setSelectedCert(null)}
            />
            <img
